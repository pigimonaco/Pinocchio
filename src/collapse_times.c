...
#define CT_NBINS_XY (50)  // 10
#define CT_NBINS_D  (100) // 20
...
int compute_collapse_times(int ismooth) {
  ...
  //  Smoothing.EqRadius[ismooth]     = Radius(global_variance); 
...
double inverse_collapse_time(int ismooth, double *deformation_tensor,
							 double *x1, double *x2, double *x3, int *fail) {
  int m;
  ...
  *fail=0;
  ...
  mu3 =  deformation_tensor[0] * deformation_tensor[1] * deformation_tensor[2]
    + 2.0 * deformation_tensor[3] * deformation_tensor[4] * deformation_tensor[5]
  ...
  if (q==0.) {
	...
	*x3=deformation_tensor[2];
	...
	r = -(2.0 * mu1 * mu1 * mu1 - 9.0 * mu1 * mu2 + 27.0 * mu3) / 54.0;
	...
	sq = sqrt(q);
	t  = acos(r / q / sq);
	*x1 = -2.0 * sq * cos(t / 3.0) + mu1 / 3.0;
	*x2 = -2.0 * sq * cos((t + 2.0*PI) / 3.0) + mu1 / 3.0;
	*x3 = -2.0 * sq * cos((t + 4.0*PI) / 3.0) + mu1 / 3.0;
  }
  ...
  if (t2>0.9) {
	...
	fprintf(JUNK,"%d  %f %f %f   %f %f %f   %20g %20g %20g\n",ismooth, (*x1+*x2+*x3)/ampl,
			(*x1-*x2)/ampl, (*x2-*x3)/ampl, *x1, *x2, *x3, t, t2, t-t2);
#endif
  }
...
int initialize_collapse_times(int ismooth) {
  ...
  if (!ismooth) {
	...
	if (CT_EXPO==1) {
	  ...
	  interval = 2.0 * CT_RANGE_D / (double)CT_NBINS_D;
  ...
  //  if (!ismooth)
  {
	...
	MPI_Allgatherv( MPI_IN_PLACE, counts[ThisTask], MPI_DOUBLE, CT_table,
	...
	for (i=0; i<CT_NBINS_XY; i++)
	  for (j=0; j<CT_NBINS_XY; j++)	{
		gsl_spline_init(CT_Spline[i][j], delta_vector,
...
double ell_sng(int ismooth, double l1, double l2, double l3) {
  ...
  if (ismooth<Smoothing.Nsmooth-1)
	...
	//	ode_param = Smoothing.EqRadius[ismooth];
#endif
