...
int mybsearch(int *, unsigned int, unsigned int);
...
static int index_compare_F(const void * a,const void * b)
{
  ...
  int flag1 = (frag[A].Fmax < frag[B].Fmax);  // returns 1
  int flag_1 = (frag[A].Fmax > frag[B].Fmax);   // returns -1

  /* comment [LT]
   *
   * this routine return 1 if a < b and -1 if a > b
   * that is unusual, normally these helper functions return
   * the opposite.
   * This means that the ordering will be in descending order
   */
...
int mybsearch(int *data, unsigned int N, unsigned int Key)
 {  
   int register low = 0;
   int register high = N;
   int register mid; 
...
int find_location(int i,int j,int k)
{
  ...
  int nn = mybsearch(sorted_pos, subbox.Nstored, pos);

  /* if the particle is not stored, it returns -1 */
  return nn;
}
