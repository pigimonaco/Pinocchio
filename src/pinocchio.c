  ...
  if (!ThisTask)
    {
      ...
      printf("the Gravity used here is Cubic Galileon, and the background evolution obeys the Cubic Galileon, so you can ignore the wCDM model that used as the background for LCDM and fR ");
#endif
#endif
#ifdef READ_PK_TABLE
      printf("Scales will range from %10g to %10g 1/Mpc, in %d steps\n",
