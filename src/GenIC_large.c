...
int GenIC_large(int ThisGrid)
{
  ...
  dprintf(VDIAG, ThisTask, "==== %d in k-space:: (%td, %td) - (%td, %td) - (%td, %td)\n",
	  ThisTask,
...
int generate_seeds_plane(int ThisGrid, map_point bottom_left, map_point top_right, unsigned Nmesh, int k0symmetries)
{
  ...
  dprintf(VDIAG, ThisTask, "Task %d: subregions done\n", ThisTask); fflush(stdout);
  ...
  if(internal.verbose_level > VDIAG)
	...
	if( ThisTask == T )
	  for(int i = 0; i < N_subregions; i++)
	    {
	      dprintf(VDIAG, ThisTask, "\ttask: %3d\n"
  ...
  dprintf(VDIAG, ThisTask, "Task %d: seeds generated\n", ThisTask); fflush(stdout);
...
/* ---------------------------------------
  ...
  dprintf(VDIAG, ThisTask, "Task %d: symmetries generated\n", ThisTask); fflush(stdout);
...
// note: by construction single regions are supposed to lie within quadrants
{
  ...
  dprintf(VDIAG, ThisTask, "\t\t Task %d: region [%d, %d] -> "
  ...
  for(int tt = 0; tt < NTasks; tt++)
      ...
      if( ThisTask == tt)
	{
	  if( z0 == 0)
	    ...
	    dprintf(VDIAG, ThisTask, "\t\t\tTask %d skips because z0 == %d\n", ThisTask, z0);
	}
      MPI_Barrier(MPI_COMM_WORLD);
    }
