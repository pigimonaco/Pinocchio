...
int set_parameters() {
  ...
  params.ParticleMass  = 2.775499745e11 * params.Hubble100 * params.Hubble100
	* params.Omega0 * pow(params.InterPartDist, 3.);
...
int set_smoothing() {
  ...
  var_min    = pow(1.686 / NSIGMA / GrowingMode(outputs.zlast, 0.), 2.);
  /* rmax       = Radius(var_min); */
  ...
  //  Smoothing.EqRadius    = (double*)malloc(Smoothing.Nsmooth * sizeof(double));
  ...
  if (!ThisTask)
    for (ismooth=0; ismooth<Smoothing.Nsmooth; ismooth++)
      printf("           %2d)  Radius=%10f, Variance=%10f\n", ismooth+1,
			 Smoothing.Radius[ismooth], Smoothing.Variance[ismooth]);
...
double myz;
int cone_and_cube_intersect(double *, double *, double *, double *,
...
int set_plc(void) {
  ...
  for (ir=-NAll; ir<=NAll; ir++)
    for (jr=-NAll; jr<=NAll; jr++)
      for (kr=-NAll; kr<=NAll; kr++) {
		...
		intersection = cone_and_cube_intersect(x, l, plc.center, plc.zvers, params.PLCAperture,
											   &smallestr, &largestr, &axis);
		...
		if (intersection && !(smallestr>Largest_r || largestr<Smallest_r)) {
		  if (!ThisTask)
			fprintf(fout," %3d  %3d %3d %3d   %10.6f %10.6f   %d  %d\n", this, ir, jr, kr,
					smallestr, largestr, intersection,axis);
		  plc.repls[this].i    = ir;
		  plc.repls[this].j    = jr;
		  plc.repls[this].k    = kr;
  ...
  if (!ThisTask) {
	...
	printf("Past Light Cone will be centred on point [%f,%f,%f] (true Mpc)\n",
		   plc.center[0] * params.InterPartDist,
...
double maxF(double *P, double *V, double *U, double *D, double *L) {
  ...
  return (cosDP + tmax * cosDU) / sqrt(1. + tmax * tmax + 2. * tmax * cosUP);
}

int cone_and_cube_intersect(double *Oc, double *L, double *V, double *D, double theta,
							double *rmin, double *rmax, int *axis) {
  ...
  *rmax = 0.;
  ...
  for (i=0;i<2;i++)
    for (j=0;j<2;j++)
      for (k=0;k<2;k++) {
		r = sqrt(pow(Oc[0] + i * L[0] - V[0], 2.) +
				 pow(Oc[1] + j * L[1] - V[1], 2.) +
				 pow(Oc[2] + k * L[2] - V[2], 2.));
		if (r>*rmax)
		  *rmax = r;
	  }


  /* Min distance from and intersection of axis with cube faces */
  *axis = 0;
  for (dim=0; dim<3; dim++)  /* three dimension (normals to cube faces) */
    for (i=0; i<2; i++) {    /* two faces per dimension */
	  ...
	  if (V[dim1]<Oc[dim1])               /* only of their projection is outside the face */
		r += pow(V[dim1] - Oc[dim1], 2.);
	  else if (V[dim1]>=Oc[dim1]+L[dim1])
		r += pow(V[dim1] - Oc[dim1] - L[dim1], 2.);
	  if (V[dim2]<Oc[dim2])
		r += pow(V[dim2] - Oc[dim2], 2.);
	  else if (V[dim2]>=Oc[dim2]+L[dim2])
		r += pow(V[dim2] - Oc[dim2] - L[dim2], 2.);
	  r = sqrt(r);
	  if (r<*rmin)
		*rmin = r;
  ...
  /* step 4: compute maximum of ** F = (P-V) dot D /|P-V| - cos theta ** for each cube edge */
  Fmax = -10.;
...
int set_subboxes() {
  ...
  cc = 1. / pow(params.BoxSize_htrue, 3.);
  ...
  ssafe = 2. * subbox.safe;
  FmaxBPP = (double)sizeof(product_data) + 10. * (double)sizeof(double) + 
  ...
  FragG_BPP = 3. * (double)sizeof(int) + (double)(sizeof(group_data) + sizeof(histories_data)) / 10.;
#ifdef PLC
  ...
  mf.vol      = (double)MyGrids[0].GSglobal_x * (double)MyGrids[0].GSglobal_y *
	(double)MyGrids[0].GSglobal_z * pow(params.InterPartDist, 3.);
  mf.mmin = log10(params.MinHaloMass * params.ParticleMass) - 0.001 * DELTAM;
  mf.mmax = log10(params.Largest) + 3. * DELTAM;
  ...
  if (!ThisTask) {
	...
	printf("Number of sub-boxes per dimension: %d %d %d\n", subbox.nbox_x, subbox.nbox_y,
		   subbox.nbox_z_allslices);
