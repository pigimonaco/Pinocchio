...
FILE *cubic1;            //cubic_Galileoncccccccccccccccccccccc
FILE *cubic2;            //cubic_Galileoncccccccccccccccccccccc
...
cubic1 = fopen( "cubic1.out", "w" );//cubic_Galileoncccccccccccccccccc
cubic2 = fopen( "cubic2.out", "w" );//cubic_Galileoncccccccccccccccccc
  ...
  fclose(cubic1);    //cubic_Galileoncccccccccccccccccccccccccc
  fclose(cubic2);    //cubic_Galileoncccccccccccccccccccccccccc
...
3.*params.Omega0*params.Omega0/pow(x,7.0)/sqrt(params.Omega0*params.Omega0/pow(x,6.)+4.*params.OmegaLambda))/H/H);
  ...
  double Hlcdm,dHlcdm_dt,de_Esq,de_a1,Esq,a1,b1;
  Hlcdm=100.*params.Hubble100*sqrt(params.Omega0/pow(x,3.)+OmegaRad/pow(x,4.)+params.OmegaLambda);
  dHlcdm_dt=-100.*params.Hubble100*100.*params.Hubble100*(params.Omega0/2./pow(x,3.)+OmegaRad/pow(x,4.)-params.OmegaLambda)-Hlcdm*Hlcdm;

  de_Esq = 1.;
  de_a1  = 0.;
  
  Esq = params.Omega0/pow(x,3.0)+params.OmegaLambda*de_Esq;
  a1   = -(3./x + 0.5 *((- 3.*params.Omega0/pow(x,4.0)+ de_a1)/Esq));
  b1   = 1.5 *params.Omega0/(Esq*pow(x,5.0));


  
  fprintf(cubic1,"%12lg  %12lg  %12lg  %12lg  %12lg  %12lg  %12lg  %12lg  %12lg  %12lg\n",x,H/Hlcdm,
dHdt/dHlcdm_dt,dphidt,ddphiddt,x1,x3,epsilon,inter,Qs);
  fprintf(cubic2,"%12lg  %12lg  %12lg  %12lg  %12lg  %12lg  %12lg  %12lg\n",x,cs2,lambda2,zeta,e1/a1,b2/b1,e2/b2,e3/b1);
  //fprintf(cubic1,"%12lg\n",inter);
  //fprintf(cubic2,"%12lg  %12lg\n",x,cs2);
  ...
  if (params.Sigma8!=0.0 && WhichSpectrum!=5)
    {
      ...
      PkNorm = tmp;
      //PkNorm=ComputeMassVariance(8.0/params.Hubble100);//Cubic_Galileon
      if (!ThisTask)
	{
          printf("********************************************************************%d\n",ThisTask);
	  if (WhichSpectrum==2)
