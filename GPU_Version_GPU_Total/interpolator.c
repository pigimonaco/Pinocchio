...
// #endif
    ...
    // Allocate memory for CubicSpline struct
    CubicSpline* spline_custom = cubic_spline_alloc(N);

    // Initialize CubicSpline struct
    custom_cubic_spline_gpu(x_data, y_data, N, spline_custom);
    printf(spline_custom);
    // Interpolate at all x values using your custom cubic spline interpolation
    double interpolated_cubic_spline[N];

    #pragma omp target map(to: x_data[0:N], spline_custom[0:1]) map(from: interpolated_cubic_spline[0:N])
    #pragma omp teams distribute parallel for
    for (int i = 0; i < N; ++i) {
        interpolated_cubic_spline[i] = custom_cubic_spline_eval(spline_custom, x_data[i]);
    }
