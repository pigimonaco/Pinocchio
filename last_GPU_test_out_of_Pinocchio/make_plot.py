...
plt.figure(figsize=(12, 6))
# plt.title('Comparison of Spline Interpolations: 30 points')
plt.suptitle('Comparison of Spline Interpolations: 1000 points')
plt.subplot(1, 2, 1)
...
plt.plot(x_data, y_data, label="Tabulated value", color='black', linestyle='-')
# plt.scatter(sorted_evaluated_values, sorted_actual_values, label="Real values", marker='o', s=80, edgecolors='blue', facecolors=None)
...
plt.scatter(sorted_evaluated_values, sorted_cubic_spline, label="Custom Spline Interpolation", marker='^', color='red' )
plt.xlabel('x')
plt.ylabel('f(x)')
plt.legend()

plt.subplot(1, 2, 2)
residuals = (np.array(sorted_gsl_spline) - np.array(sorted_cubic_spline))/(np.array(sorted_gsl_spline))
...
plt.savefig('1000_points_CUSTOM_vs_GSL.png')
